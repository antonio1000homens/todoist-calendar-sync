...
test("project mapping comment recovery restores missing progress state", () => {
  const source = mapping({
    calendarProgressVersion: 1,
    completedThroughOriginalStart: "2099-09-17",
  });
  const parsed = parseMappingComment({
    id: "comment-progress",
    content: serializeMappingComment(source, "project-home", undefined, 3),
  });
  assert.ok(parsed && !("error" in parsed));

  const recovered = recoverMappingFromProjectComment(mapping({
  ...
  assert.equal(recovered.recurrenceOwner, "calendar");
  assert.equal(recovered.seriesId, "series-1");
  assert.equal(recovered.masterEventId, "master-1");
  assert.equal(recovered.activeInstanceId, "event-1");
  assert.equal(recovered.calendarProgressVersion, 1);
  assert.equal(recovered.completedThroughOriginalStart, "2099-09-17");
});

test("project mapping comment recovery never lowers an existing completion watermark", () => {
  const parsed = parseMappingComment({
    id: "comment-stale-progress",
    content: serializeMappingComment(mapping({
      calendarProgressVersion: 1,
      completedThroughOriginalStart: "2099-09-17",
    }), "project-home", undefined, 3),
  });
  ...
  const recovered = recoverMappingFromProjectComment(mapping({
    calendarProgressVersion: 1,
    completedThroughOriginalStart: "2099-09-19",
  }), parsed);
  assert.equal(recovered.completedThroughOriginalStart, "2099-09-19");
});
